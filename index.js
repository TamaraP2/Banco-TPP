...
document.querySelector(".info-btn").addEventListener("click", function() {
    ...
    if (!document.querySelector(".info-box")) {
        ...
        document.querySelector(".login-btn").insertAdjacentHTML("afterend", `<div class="info-box"><p class="info-box-titulo">CONTAS DISPONÍVEIS</p></div>`);

        contas.forEach(conta => {
            document.querySelector(".info-box").insertAdjacentHTML("beforeend", `<p><span class="info-box-contas">Usuário: ${conta.username}</span> <span class="info-box-contas">Senha: ${conta.password}</span></p>`);
        });
...
function verificadorLogin(usernameLogin, passwordLogin) {
    ...
    if (usuarioAutenticado === false && !document.querySelector(".usuario-incorreto")) {
        document.querySelector(".login-titulo").insertAdjacentHTML("afterend", "<p class='usuario-incorreto'>Usuário ou senha incorretos. Por favor, tente novamente</p>");
    }
